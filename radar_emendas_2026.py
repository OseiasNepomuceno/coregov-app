...
def exibir_radar():
    ...
    try:
        # Carregamento e limpeza rigorosa dos nomes das colunas
        df = pd.read_csv(nome_arquivo, sep=None, engine='python', encoding='latin1', on_bad_lines='skip', dtype=str)
        df.columns = [str(c).replace('"', '').strip() for c in df.columns]

        # 1. IDENTIFICAÇÃO DINÂMICA DE COLUNAS (Para evitar o KeyError)
        # Procura a coluna de Valor Pago independente de maiúsculas/minúsculas
        col_pago = next((c for c in df.columns if c.upper() == "VALOR PAGO"), None)
        col_empenhado = next((c for c in df.columns if "EMPENHADO" in c.upper()), None)
        col_liquidado = next((c for c in df.columns if "LIQUIDADO" in c.upper()), None)
        col_autor = next((c for c in df.columns if "NOME DO AUTOR" in c.upper()), None)
        ...
        # 3. FILTRO DE ANO
        col_ano = next((c for c in df.columns if "ANO DA EMENDA" in c.upper()), None)
        if col_ano:
            df = df[df[col_ano].astype(str).str.contains("2026")].copy()

        # 4. CONVERSÃO FINANCEIRA (Segura)
        ...
        # 5. DEFINIÇÃO DAS COLUNAS DE EXIBIÇÃO
        colunas_exibir = [col_ano, "Tipo de Emenda", col_autor, col_local, "UF", "Região", "Nome Programa", col_empenhado, col_liquidado, col_pago]
        df_exibir = df[[c for c in colunas_exibir if c and c in df.columns]].copy()
    ...
    if not df_exibir.empty:
        ...
        if tipo_visao == "Visão Geral":
            ...
            with c1: st.metric("💰 Empenhado", formatar_moeda(df_exibir[col_empenhado].sum() if col_empenhado else 0))
            with c2: st.metric("💸 Liquidado", formatar_moeda(df_exibir[col_liquidado].sum() if col_liquidado else 0))
            with c3: st.metric("✅ Pago", formatar_moeda(df_exibir[col_pago].sum() if col_pago else 0))
            
            st.divider()
            if col_local in df_exibir.columns and col_pago:
            ...
            # --- VISÃO POR FAVORECIDO (GRÁFICOS PROTEGIDOS) ---
            st.divider()
            cl, cr = st.columns(2)
            with cl:
                if col_autor and col_pago:
            ...
            with cr:
                col_prog = next((c for c in df_exibir.columns if "PROGRAMA" in c.upper()), None)
                if col_prog and col_pago:
                    df_p = df_exibir.groupby(col_prog)[col_pago].sum().sort_values(ascending=False).head(8).reset_index()
                    fig2 = px.pie(df_p, names=col_prog, values=col_pago, title="Distribuição por Programa", hole=0.4)
